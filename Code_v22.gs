// ============================================================
// GREEN SALON — BILLING SYSTEM v22
...
function updateMonthly(branchId,entryType,staffName,svcAmt,tipAmt,payment,prodAmt,expAmt){
  ...
  let dr=-1,tr=-1;
  if(lr>=3){
    const dateVals=sh.getRange(3,1,lr-2,1).getValues();
    dateVals.forEach((row,idx)=>{const v=String(row[0]).trim();if(v===dt)dr=idx+3;if(v==="TOTAL")tr=idx+3;});
  }
  ...
  if(dr<0){
    ...
    if(tr>0){
      ...
      dr=tr;tr=tr+1; // TOTAL shifted down
    }else{
...
function rebuildTotal(sh,nc){
  ...
  for(let r=3;r<=last;r++){if(String(sh.getRange(r,1).getValue()).trim()==="TOTAL"){tr=r;break;}}
...
function addStaff(d){const sh=branchTab(d.branchId,"Staff");const id="S"+Date.now();sh.appendRow([id,d.name,d.pin||"0000",d.commissionPct||0,d.hasCommission!==false,d.photoUrl||"",true]);const ss=branchSS(d.branchId);const bn=branchDisplayName(d.branchId);ensureStaffInDaily(ss,d.name);const msh=ss.getSheetByName(monthName());if(msh)ensureStaffInMonthly(msh,d.name,bn);return{staffId:id};}
...
function renameStaff(d){if(!d.newName)throw new Error("New name required");const sh=branchTab(d.branchId,"Staff"),rows=sh.getDataRange().getValues();for(let i=1;i<rows.length;i++){if(rows[i][0]===d.staffId){const old=rows[i][1];sh.getRange(i+1,2).setValue(d.newName);return{oldName:old,newName:d.newName};}}throw new Error("Staff not found");}
...
function getBranchSummary(d){
  ...
  eR.slice(1).forEach(r=>{if(String(r[2])===dt&&r[10]!==true&&r[10]!=="TRUE"){totalEntries++;totalRevenue+=Number(r[6])||0;totalTips+=Number(r[7])||0;}});
  // Include product sales in revenue
  const pR=branchTab(bid,"ProductSales").getDataRange().getValues();
  let totalProducts=0;
  pR.slice(1).forEach(r=>{if(String(r[2])===dt&&r[8]!==true&&r[8]!=="TRUE")totalProducts+=Number(r[6])||0;});
